-- Fix: prevent duplicates for same manager/date in save_crm_report
-- and clean up empty duplicate reports created previously.
...
BEGIN
  ...
  INSERT INTO public.crm_confirmed_orders (report_id, entity_name, bin, order_count, amounts, total_amount, sort_order)
  SELECT
    v_report_id,
    trim(coalesce(raw."entityName", '')),
    trim(coalesce(raw.bin, '')),
    ...
    row_number() OVER () - 1
  FROM jsonb_to_recordset(v_orders) AS raw("entityName" text, bin text, "orderCount" int, amounts numeric[], "totalAmount" numeric)
  WHERE trim(coalesce(raw."entityName", '')) <> ''
    AND trim(coalesce(raw.bin, '')) <> '';
...
-- Cleanup only empty duplicate rows (safe cleanup).
WITH dup AS (
  SELECT manager, report_date, array_agg(id ORDER BY (processed_total + new_in_work + calls_total + validated_total) DESC, id DESC) AS ids
  FROM public.crm_reports
  GROUP BY manager, report_date
  HAVING count(*) > 1
),
losers AS (
...
empty_losers AS (
  SELECT l.id
  FROM losers l
  WHERE NOT EXISTS (SELECT 1 FROM public.crm_assigned_meetings am WHERE am.report_id = l.id)
    AND NOT EXISTS (SELECT 1 FROM public.crm_conducted_meetings cm WHERE cm.report_id = l.id)
    AND NOT EXISTS (SELECT 1 FROM public.crm_confirmed_orders co WHERE co.report_id = l.id)
)
DELETE FROM public.crm_reports r
WHERE r.id IN (SELECT id FROM empty_losers);
