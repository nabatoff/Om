-- Баг: лидоруб передаёт лид с галочкой «Встреча» -> digger_transfer_enterprise_batch создаёт
-- встречу типа «Крупный лид» в СОБСТВЕННОМ отчёте лидоруба (report.manager_id = сам лидоруб).
-- Триггер на crm_assigned_meetings видит тип «Крупный лид» и вызывает
-- sync_enterprise_lead_for_krup_meeting(bin, p_manager_id = <лидоруб>, ...), которая пытается
-- обработать лидоруба как назначенного менеджера и падает в ensure_crm_report_for_manager
-- ('Менеджер не найден', т.к. роль лидоруба — lead_digger, а не manager).
-- Фикс: функция должна работать только для реальных менеджеров; для любого другого
-- p_manager_id просто ничего не делать.
CREATE OR REPLACE FUNCTION public.sync_enterprise_lead_for_krup_meeting(p_bin text, p_manager_id uuid, p_hint_assigned_id uuid DEFAULT NULL::uuid, p_hint_date date DEFAULT NULL::date)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.crm_conducted_meetings cm
    JOIN public.crm_reports r ON r.id = cm.report_id
    WHERE cm.deleted_at IS NULL AND r.manager_id = p_manager_id
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.crm_assigned_meetings m
  ...
  SELECT m.id, m.meeting_date INTO v_assigned_id, v_meeting_date
  FROM public.crm_assigned_meetings m
  JOIN public.crm_reports r ON r.id = m.report_id
  WHERE m.deleted_at IS NULL AND r.manager_id = p_manager_id
    AND regexp_replace(coalesce(m.bin, ''), '[^0-9]', '', 'g') = v_bin
    AND lower(replace(coalesce(m.meeting_type, ''), 'ё', 'е')) LIKE '%крупн%'
  ...
  IF v_assigned_id IS NULL AND p_hint_assigned_id IS NOT NULL THEN
    IF EXISTS (
      SELECT 1 FROM public.crm_assigned_meetings m
      WHERE m.id = p_hint_assigned_id AND m.deleted_at IS NULL
        AND lower(replace(coalesce(m.meeting_type, ''), 'ё', 'е')) LIKE '%крупн%'
    ) THEN
  ...
  IF NOT v_has_conducted AND v_assigned_id IS NULL THEN
    ...
    FROM public.crm_assigned_meetings m
    JOIN public.crm_reports r ON r.id = m.report_id
    WHERE m.deleted_at IS NOT NULL AND r.manager_id = p_manager_id
      AND regexp_replace(coalesce(m.bin, ''), '[^0-9]', '', 'g') = v_bin
      AND lower(replace(coalesce(m.meeting_type, ''), 'ё', 'е')) LIKE '%крупн%'
    ...
    IF v_assigned_id IS NOT NULL THEN
      UPDATE public.crm_assigned_meetings SET deleted_at = NULL WHERE id = v_assigned_id;
      v_had_scheduled := true;
    END IF;
  END IF;
  ...
  WHERE l.bin = v_bin AND l.routing_status IN ('pending_distribution', 'assigned_to_manager')
  ORDER BY
    CASE WHEN l.assigned_manager_id IS NOT DISTINCT FROM p_manager_id THEN 0
         WHEN l.assigned_manager_id IS NULL THEN 1 ELSE 2 END,
    l.created_at DESC
  LIMIT 1;
  ...
  SET business_scale = 'enterprise', manager_id = coalesce(manager_id, p_manager_id)
  WHERE bin = v_bin;
  ...
  IF v_lead.id IS NULL THEN
    ...
    INSERT INTO public.crm_enterprise_leads (
      ...
      v_bin, coalesce(v_digger_id, p_manager_id), p_manager_id, 'assigned_to_manager', v_new_status,
      now(), v_meeting_date, now(), v_assigned_id,
      v_assigned_id IS NOT NULL OR v_had_scheduled, true
    ) RETURNING * INTO v_lead;

    INSERT INTO public.crm_lead_events (lead_id, actor_id, action, payload)
    VALUES (v_lead.id, auth.uid(), 'meeting_sync',
      jsonb_build_object('manager_id', p_manager_id, 'meeting_status', v_new_status, 'source', 'krup_meeting'));
    RETURN;
  END IF;
  ...
  IF v_prev_routing = 'pending_distribution' AND NOT coalesce(v_lead.qualification_counted, false) THEN
    ...
    UPDATE public.crm_reports SET validated_total = coalesce(validated_total, 0) + 1 WHERE id = v_report_id;
    v_count_qual := true;
  END IF;
  ...
  IF v_prev_routing IS DISTINCT FROM 'assigned_to_manager'
    ...
    INSERT INTO public.crm_lead_events (lead_id, actor_id, action, payload)
    VALUES (
      v_lead.id, auth.uid(), 'meeting_sync',
      jsonb_build_object(
