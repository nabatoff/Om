...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      -- Do NOT block when sales manager_id is set — digger_id coexists
      UPDATE public.crm_clients
      SET
        name = v_name,
...
BEGIN
  ...
  IF v_lead.routing_status <> 'assigned_to_manager' THEN
    RAISE EXCEPTION 'Вернуть можно только назначенный лид' USING ERRCODE = '23514';
  END IF;
  ...
  SET
    ...
    returned_at = now(),
    meeting_status = coalesce(meeting_status, 'cancelled'),
    updated_at = now()
  WHERE id = p_lead_id;
...
BEGIN
  ...
  DELETE FROM public.crm_enterprise_leads
  WHERE id = p_lead_id
    AND routing_status = 'returned_to_smb';

  IF NOT FOUND THEN
    RAISE EXCEPTION 'Возврат не найден' USING ERRCODE = 'P0002';
  END IF;
END;
$$;

CREATE OR REPLACE FUNCTION public.admin_clear_returned_leads()
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  WITH deleted AS (
    DELETE FROM public.crm_enterprise_leads
    WHERE routing_status = 'returned_to_smb'
    RETURNING 1
  )
...
REVOKE ALL ON FUNCTION public.admin_delete_returned_lead(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_delete_returned_lead(uuid) TO authenticated;
REVOKE ALL ON FUNCTION public.admin_clear_returned_leads() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_clear_returned_leads() TO authenticated;
