-- Лидоруб хочет видеть по каждому переданному лиду: подтвердил ли контрагент заказ,
-- на какую сумму и сколько заказов. Добавляем агрегат по crm_confirmed_orders (join по БИН,
-- как и в остальной KPI-логике) в list_enterprise_leads.
-- Меняем набор выходных колонок -> CREATE OR REPLACE не годится, нужен явный DROP.
