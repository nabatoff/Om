-- Удаление проведённой «Крупный лид»: статус в «Переданные в круп»
--   был план встречи → Ожидает встречи (NULL)
--   плана не было → Передан (in_work)
...
BEGIN
  ...
  IF v_lead.id IS NULL THEN
    v_new_status := CASE
      ...
      WHEN v_assigned_id IS NOT NULL OR v_had_scheduled THEN NULL
      ELSE 'in_work'
    END;
