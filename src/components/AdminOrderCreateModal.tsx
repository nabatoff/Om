...
import { AlertTriangle, CheckCircle, Plus, Save, UserPlus, X } from 'lucide-react';
import type { UiClient, UiManagerProfile, UiOrder } from '../lib/crmApi';
import { adminCreateConfirmedOrder } from '../lib/crmApi';
...
export function AdminOrderCreateModal({
  ...
  const [reportDate, setReportDate] = useState(() => formatYmdLocal(new Date()));
  const [draft, setDraft] = useState<UiOrder>(emptyDraft);
  const [saving, setSaving] = useState(false);
  const [err, setErr] = useState<string | null>(null);

  const amountErr = useMemo(() => {
    if (draft.totalAmount <= 0 && !draft.amounts.some((a) => a > 0)) {
      return 'Укажите сумму заказа';
    }
  ...
  const save = async () => {
    ...
    setErr(null);
    try {
      await adminCreateConfirmedOrder({
        managerId,
        reportDate,
      ...
      setErr(e instanceof Error ? e.message : 'Не удалось создать заказ');
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="p-6 border-b border-gray-100 flex justify-between items-start gap-4">
          <div className="text-left">
            ...
            <p className="text-[10px] text-gray-500 font-bold uppercase mt-1">
              Ручное создание · привязка к менеджеру
            </p>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-5 flex-1 min-h-0 text-left">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              >
                {managers.length === 0 ? <option value="">Нет менеджеров</option> : null}
                {managers.map((m) => (
          ...
          <ContractorField
            ...
            clients={clients}
            datalistId="admin-order-create-counterparty"
          ...
          <div className="space-y-1">
            <ContractorField
              ...
              clients={clients}
              datalistId="admin-order-create-via"
